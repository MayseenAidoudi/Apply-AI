...
export class MyBackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const statusCheckingLambda = new lambda.Function(
      this,
      "StatusCheckingLambda",
      {
        runtime: lambda.Runtime.PYTHON_3_9,
    ...
    MainScrapingLambda.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["ssm:GetParameter"],
        resources: [
          `arn:aws:ssm:${this.region}:${this.account}:parameter/FIRECRAWL_API_KEY`,
        ],
      })
    );
    ...
    aiProcessingLambda.addToRolePolicy(
      new iam.PolicyStatement({
    ...
    const api = new apigateway.RestApi(this, "JobScraperApi", {
      ...
      description:
        "This service scrapes job offers and provides AI-powered insights.",
    });

    // Create scrapeAndGenerate resource
    const scrapeResource = api.root.addResource("scrapeAndGenerate");
    scrapeResource.addMethod(
      "POST",
      new apigateway.LambdaIntegration(MainScrapingLambda),
      {
        methodResponses: [{ statusCode: "200" }],
      }
    );

    // Add CORS to scrapeAndGenerate resource
    scrapeResource.addCorsPreflight({
      allowOrigins: ["*"],
      allowMethods: ["POST"],
    ...
    statusResource.addMethod(
      "GET",
      new apigateway.LambdaIntegration(statusCheckingLambda),
      {
        methodResponses: [{ statusCode: "200" }],
      }
    );

    // Add CORS to status resource
